...
def access_denied(message, file_path):
    time = datetime.datetime.now()
    log = f"{time.day}.{time.month}.{time.year} {time.hour}:{time.minute}:{time.second}:{int(time.microsecond / 1000)} [ACCESS DENIED] {message}"
    with open(file_path, "a", encoding="utf-8") as logfile:
        logfile.write(f"{log}\n")